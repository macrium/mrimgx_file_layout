...
{
	enum class DriveType { LOCAL, NETWORK, CD, BLOCKSTORE };

	enum class DiskFormat { eMBR, eGPT, eDynamic, eUnknown };

	enum class BackupType { eFull, eIncremental, eDifferential, eAuto };

	enum class FilenamePrefixType { ePrefixImageID, ePrefixISODate };

	enum class CompressionType { eNone, eMedium, eHigh };

	enum class AES { eNone, eStandard, eMedium, eHigh };

	enum class KeyDerivation { ePBKDF2 };

	enum class FileType { ePartition, eFileAndFolder };
	...
	enum class CompressionMethod { eZStd };

	enum class ConsolidationType { eNone, eDaily, eWeekly, eMonthly, eSyntheticFull, eIncrementalMerge };

	enum class CPUPrority { eIdle = 0, eBelowNormal = 1, eNormal = 3, eAboveNormal };

	enum class RetentionType { eRetentionFull, eRetentionDiff, eRetentionInc };

	enum class RetentionPeriod { eRetainCount, eRetainDays, eRetainWeeks };

	enum class PartitionType { Unallocated, Primary, Logical };

	enum class ShutdownLevel { eNone = -1, eShutdown, eHibernate, eSuspend, eReboot };

	enum class BitLocker { eBitLockerNone, eBitLockerLocked, eBitLockerUnlocked };

	enum class BackupFileType { eUndefinedFileType = -1, eImage = 0, eBackup, eExchange, eSQLServer, eClone };

	enum class Bootable { eNotSet, eYes, eNo };

	enum class FileAndFolderMatchType { eSingleMatch, eStrict, eAny };
